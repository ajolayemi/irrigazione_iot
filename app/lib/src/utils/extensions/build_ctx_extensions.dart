...
extension BuildContextExtensions on BuildContext {
  ...
  String customFormatDateTime({required String timeAgoDateString, required DateTime dateTime}) =>
  ...
  Future<bool> showDismissalDialog({String? alternateDialog, String? where, String? alternateTitle}) async {
    final loc = this.loc;
    return await showAlertDialog(
          ...
          content: alternateDialog ?? loc.deleteConfirmationDialogTitle(where ?? ''),
        ) ??
        false;
  }

  Future<bool> showSaveUpdateDialog({required bool isUpdating, required String what}) async {
    final loc = this.loc;
    return await showAlertDialog(
          context: this,
          title: isUpdating ? loc.formGenericUpdateDialogTitle : loc.formGenericSaveDialogTitle,
          content: isUpdating ? loc.formGenericUpdateDialogContent(what) : loc.formGenericSaveDialogContent(what),
          defaultActionText: isUpdating ? loc.genericUpdateButtonLabel : loc.genericSaveButtonLabel,
          cancelActionText: loc.alertDialogCancel,
        ) ??
        false;
  }

  Future<bool> showStatusToggleDialog({required bool status, required String what}) async {
    final loc = this.loc;
    return await showAlertDialog(
          context: this,
          title: loc.genericAlertDialogTitle,
          content: status ? loc.onStatusUpdateAlertDialogContent(what) : loc.offStatusUpdateAlertDialogContent(what),
          defaultActionText: status ? loc.onStatusDialogConfirmButtonTitle : loc.offStatusDialogConfirmButtonTitle,
          cancelActionText: loc.alertDialogCancel,
        ) ??
        false;
  }
  ...
  Future<RadioButtonItem?> showAssignRoleDialog(String currentAssignedRole) async {
    ...
    roles.removeWhere((role) => role == CompanyUserRole.owner || role == CompanyUserRole.superuser);
            ...
            children:
                roles.map((role) {
                  return ResponsiveRadioListTile(
                    title: role.name,
                    value: RadioButtonItem(value: role.name, label: role.name),
                    groupValue: RadioButtonItem(value: currentAssignedRole, label: currentAssignedRole),
  ...
  void showSnackBar({required String message, Duration duration = const Duration(seconds: 5)}) {
