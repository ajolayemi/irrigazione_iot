...
class SharedPreferencesService {
  ...
  static const String _weenatPlotSensorDataTimestampKey = 'weenat_plot_sensor_data_timestamp';

  String _buildFinalKey({required String prefix, required String key}) {
    return '$prefix _$key';
  }
  ...
  Future<void> clearWeenatTokenPrefs({String? uid}) async {
    ...
    final prefKey = _buildFinalKey(prefix: uid, key: _weenatTokenSuffixKey);
    final timestampKey = _buildFinalKey(prefix: uid, key: _weenatTimestampTokenSuffixKey);
  ...
  Future<void> setUserWeenatToken({required String token, String? uid}) async {
    if (uid == null || uid.isEmpty) return;
    ...
    pref.setString(timestampKey, DateTime.now().toIso8601String());
  }
  ...
  Future<String?> getUserWeenatToken({String? uid}) async {
    ...
    final tokenTimestamp = DateTime.tryParse(pref.getString(timestampKey) ?? '');
  ...
  Future<void> setPlotSensorDataTimestamp({required int plotId, required DateTime timestamp}) async {
    final pref = await SharedPreferences.getInstance();
    final prefKey = _buildFinalKey(prefix: plotId.toString(), key: _weenatPlotSensorDataTimestampKey);
    pref.setString(prefKey, timestamp.toIso8601String());
  }
