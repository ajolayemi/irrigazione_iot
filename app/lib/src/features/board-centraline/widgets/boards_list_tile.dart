...
class BoardListTile extends ConsumerWidget {
  ...
  static Key boardListTileKey(Board board) => Key('boardListTileKey_${board.id}');
  ...
  Future<bool> _dismissBoard(BuildContext context, WidgetRef ref) async {
    ...
    if (await context.showDismissalDialog(where: where)) {
      return await ref.read(dismissBoardControllerProvider.notifier).confirmDismiss(board);
    }
    return false;
  }
