...
class AddUpdateCompanyController extends _$AddUpdateCompanyController {
  ...
  Future<bool> addCompany(Company? company) async {
    ...
    if (company == null) {
      state = AsyncError('no company object provided for creation', StackTrace.current);
      return Future.value(false);
    }
    state = await AsyncValue.guard(() => companyRepo.addCompany(company: company));
  ...
  Future<bool> updateCompany(Company? company) async {
    ...
    state = const AsyncLoading();
    if (company == null) {
      state = AsyncError('no company object provided for update', StackTrace.current);
      return false;
    }

    state = await AsyncValue.guard(() => companyRepo.updateCompany(company: company));
    return !state.hasError;
  }
}
