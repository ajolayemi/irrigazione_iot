...
class SectorSwitch extends ConsumerWidget {
  const SectorSwitch({super.key, required this.sector});

  final Sector sector;

  // * Keys for testing using find.byKey
  static Key sectorStatusSwitchKey(Sector sector) => Key('sectorStatusSwitchKey_${sector.id}');

  Future<void> _toggleSwitch(BuildContext context, WidgetRef ref, bool status) async {
    final canContinue = await context.showStatusToggleDialog(status: status, what: sector.name);

    if (!canContinue) return;

    ref.read(sectorSwitchControllerProvider.notifier).toggleStatus(sector, status);
  }

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final thisSectorStatusIsLoading = ref.watch(sectorSwitchControllerProvider).stateWithIdIsLoading(sector.id);
    ...
    return thisSectorStatusIsLoading
        ...
        : IgnorePointer(
          ...
          child: Switch.adaptive(
            ...
            value: isSwitchedOn,
            onChanged: (status) async => await _toggleSwitch(context, ref, status),
          ),
        );
  }
}
