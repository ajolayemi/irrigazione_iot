...
class SelectAnIrrigationSource extends StatefulWidget {
  const SelectAnIrrigationSource({super.key, this.selectedIrrigationSource});

  final String? selectedIrrigationSource;

  @override
  State<SelectAnIrrigationSource> createState() => _SelectAnIrrigationSourceState();
}
...
class _SelectAnIrrigationSourceState extends State<SelectAnIrrigationSource> {
  ...
  Widget build(BuildContext context) {
    ...
    return CustomSliverConnectSomethingTo(
      ...
      onCTAPressed: () => context.popNavigator<RadioButtonItem>(_selectedIrrigationSource),
      child: SliverList(
        delegate: SliverChildBuilderDelegate((context, index) {
          final source = irrigationSources[index];
          return ResponsiveRadioListTile(
            value: RadioButtonItem(value: source.uiName, label: source.uiName),
            groupValue: _selectedIrrigationSource,
            title: source.uiName,
            onChanged:
                (val) => setState(() {
                  _selectedIrrigationSource = _selectedIrrigationSource.copyWith(value: val?.value, label: val?.label);
                }),
          );
