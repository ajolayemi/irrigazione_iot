...
class WeenatMeasurementContainers extends ConsumerWidget {
  const WeenatMeasurementContainers({super.key, this.plotId});

  final int? plotId;

  DateTime _getStartDate({required DateTime today, required WeenatDataRangeOption option}) {
  ...
  DateTime _getEndDate({required DateTime startDate, required WeenatDataRangeOption option}) {
          ...
          return Consumer(
            builder: (context, ref, child) {
              ...
              return GestureDetector(
                onTap: () {
                  ref.read(selectedDataRangeOptionProvider.notifier).setSelected(item);
                },
                child: Consumer(
                  builder: (context, ref, child) {
                    final today = DateTime.now();
                    final cleanedToday = DateTime(today.year, today.month, today.day, today.hour);
                    final startDate = _getStartDate(today: cleanedToday, option: item);

                    final endDate = _getEndDate(startDate: startDate, option: item);
                    ...
                    return Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                          color: selectedOption == item ? AppColorsPalette.lightGreen : AppColorsPalette.grey2,
                          width: 2,
                        ),
                      ),
                      child: Column(
                        children: [
                          Text(
                            context.getLocByKey(item.locKey),
                            maxLines: 1,
                          ...
                          Text(
                            ...
                            overflow: TextOverflow.ellipsis,
                            textScaler: const TextScaler.linear(1),
                            style: textTheme.labelSmall?.copyWith(fontSize: 10, color: AppColorsPalette.grey4),
                          ),
                          const Spacer(),
                          Text(
                            '${firstData?.sensorData ?? '0'} kPa',
                            maxLines: 1,
                            overflow: TextOverflow.ellipsis,
                            textScaler: const TextScaler.linear(1),
                            style: textTheme.labelSmall?.copyWith(fontSize: 10),
