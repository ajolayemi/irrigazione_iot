...
class CollectorExpansionListTile extends ConsumerStatefulWidget {
  const CollectorExpansionListTile({super.key, required this.collector});

  final Collector collector;

  @override
  ConsumerState<CollectorExpansionListTile> createState() => _CollectorExpansionListTileState();
}

class _CollectorExpansionListTileState extends ConsumerState<CollectorExpansionListTile> {
  // Key for testing using find.byKey
  static Key collectorExpansionListTileKey(Collector collector) => Key('collectorExpansionListTileKey_${collector.id}');
  ...
  Future<bool> _dismissCollector(BuildContext context, WidgetRef ref) async {
    ...
    final shouldDismiss =
        await showAlertDialog(
          ...
          content: loc.deleteConfirmationDialogTitle(loc.nCollectorsWithArticulatedPreposition(1)),
    ...
    return ref.read(dismissCollectorControllerProvider.notifier).confirmDismiss(widget.collector.id);
  }

  @override
  Widget build(BuildContext context) {
    final collectorSectors = ref.watch(collectorSectorsStreamProvider(widget.collector.id)).valueOrNull;
