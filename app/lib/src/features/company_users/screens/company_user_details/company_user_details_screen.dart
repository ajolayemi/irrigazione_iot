...
class CompanyUserDetailsScreen extends ConsumerWidget {
  const CompanyUserDetailsScreen({super.key, required this.companyUserId});

  final String companyUserId;

  void _onTap(BuildContext context) {
    final params = PathParameters(id: companyUserId).toJson();
    context.pushNamed(AppRoute.updateCompanyUser.name, pathParameters: params);
  }

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final companyUser = ref.watch(companyUserStreamProvider(companyUserId: companyUserId));
    final loggedInUser = ref.watch(authRepositoryProvider).currentUser;
    final isMe = loggedInUser != null && loggedInUser.email == companyUser.valueOrNull?.email;
    final userCanEdit = ref.watch(userCanEditStreamProvider).valueOrNull ?? false;
        ...
        child: CustomScrollView(
          slivers: [
            AppSliverBar(
              ...
              actions: [
                CommonEditIconButton(onPressed: () => _onTap(context), alternateIsVisible: userCanEdit && !isMe),
              ],
            ),
