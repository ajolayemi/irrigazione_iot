 ...
 * @param debug A boolean that determines if the function should do some things
 * @returns boolean True if the message was successfully processed, false otherwise
 */
 ...
 * @param sectorKeys The keys for sector pressure in the message, as they appear in the message
 * @param message The message to process sector pressure from
 * @returns boolean True if the sector pressure was successfully processed, false otherwise
 */
 ...
 * It's values, if available are stored in the database
 * @param collectorPressureKeys The keys for collector pressure in the message
 * @param message The message to process collector pressure from
 * @param collectorId The id of the collector that holds the sectors in the message
 * @returns boolean True if the collector pressure was successfully processed, false otherwise
 */
 ...
 * @param terminalPressureKey The key for terminal pressure in the message
 * @param message The message to process terminal pressure from
 * @param collector_id The id of the collector that holds the sectors in the message
 * @returns boolean True if the terminal pressure was successfully processed, false otherwise
 */
const processTerminalPressure = async (terminalPressureKey, message, collector_id) => {
    try {
 ...
 * @param sectorKes The keys for the sector in the message
 * @returns The id of the collector that holds the sectors in the message if found, null otherwise
 */
const getCollectorForSector = async (sectorKeys) => {
    ...
    for (const sectorKey of sectorKeys) {
        ...
        const sector_id = await (0, read_sector_data_1.getSectorByMqttMsgName)(sectorKey);
        if (!sector_id) {
            continue;
        }
        ...
        const collector = await (0, read_sector_data_1.getCollectorBySectorId)(sector_id.id.toString());
        if (collector) {
 ...
 * @param message The message to process the keys from
 * @returns The keys for terminal pressure, collector pressures and sector pressures
 */
