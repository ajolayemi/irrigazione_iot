...
import {PressureMessageKeys} from "../interfaces/interfaces";
import {
...
const processCollectorPressure = async (
  collectorPressureKeys: Array<string>,
  message: CustomJSON,
  collectorId: number
): Promise<boolean> => {
  try {
...
const getCollectorForSector = async (
  ...
  for (const sectorKey of sectorKeys) {
    ...
    const sector_id = await getSectorByMqttMsgName(sectorKey);
    ...
    const collector = await getCollectorBySectorId(sector_id.id.toString());
